...
namespace Identity.Infrastructure.Services.Implements
{
    public class AuthenticateService : IdentityServiceBase, IAuthenticateService
    {
        ...
        public AuthenticateService(IAccountService accountService, IMapper mapper, IdentityDbContext db, UserManager<Users> userManager) : base(db, userManager)
        {
            _mapper = mapper;
        ...
        private AuthenticateViewModel GetAuthResponseModel(Users user)
        {
            try 
            {
                ...
                if (saveSucceed)
                {
                    ...
                    authModel.AccountTypeFid = ((int)AccountTypeEnum.User).ToString();
        ...
        public BaseResponse<AuthenticateViewModel> AuthenticateByFaceBook(string facebookAccessToken, string userId)
        {
            try
            {
                ...
                var syncSucceed = SyncUserProfileIfNotExisted(facebookAuthModel.Email,facebookAuthModel.FirstName,facebookAuthModel.LastName);
                if(syncSucceed)
                {
