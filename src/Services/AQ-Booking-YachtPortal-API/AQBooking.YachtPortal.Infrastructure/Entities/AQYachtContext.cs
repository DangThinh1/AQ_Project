...
namespace AQBooking.YachtPortal.Infrastructure.Entities
{
    public partial class AQYachtContext : DbContext
    {
        ...
        public virtual DbSet<Debugs> Debugs { get; set; }
        ...
        #region Function
        [DbFunction("fnYachtImageIDVal", Schema = "dbo")]
        public int? GetfnYachtImageIDVal(int YachtId, int FileType)
        { throw new NotImplementedException(); }
        [DbFunction("fnYachtInfomationVal", Schema = "dbo")]
        public string GetfnYachtInfomationVal(int YachtId)
        { throw new NotImplementedException(); }
        [DbFunction("fnYachtPricingPlanDetailVal", Schema = "dbo")]
        public string GetfnYachtPricingPlanDetailVal(int YachtId, int PricingTypeFID)
        { throw new NotImplementedException(); }
        public int GetfnYachtMerchantImageIDVal(int MerchantFId, int FileType)
        { throw new NotImplementedException(); }
        public IQueryable<YachtAttributeResults> GetfnYachtAttributeVal(int YachtID) =>
               Query<YachtAttributeResults>().FromSql($"SELECT * FROM [fnYachtAttributesTbl]({YachtID})");
        #endregion Function
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<Debugs>(entity =>
            {
            ...
            modelBuilder.HasDbFunction(this.GetType().GetMethod("GetfnYachtImageIDVal"), options =>
            {
                options.HasName("fnYachtImageIDVal");
                options.HasSchema("dbo");
            });
            modelBuilder.HasDbFunction(this.GetType().GetMethod("GetfnYachtInfomationVal"), options =>
            {
                options.HasName("fnYachtInfomationVal");
                options.HasSchema("dbo");
            });
            modelBuilder.HasDbFunction(this.GetType().GetMethod("GetfnYachtPricingPlanDetailVal"), options =>
            {
                options.HasName("fnYachtPricingPlanDetailVal");
                options.HasSchema("dbo");
            });
            modelBuilder.HasDbFunction(this.GetType().GetMethod("GetfnYachtMerchantImageIDVal"), options =>
            {
                options.HasName("fnYachtMerchantImageIDVal");
                options.HasSchema("dbo");
            });
            #endregion
            ...
            modelBuilder.Entity<Yachts>(entity =>
            {
                entity.HasOne(k => k.Option).WithOne(k => k.Yacht).HasForeignKey<YachtOptions>(k => k.YachtId);
            });

            modelBuilder.Entity<YachtMerchants>(entity =>
            {
                entity.HasMany(k => k.Yachts).WithOne(k => k.Merchant).HasForeignKey(k => k.MerchantFid);
                entity.HasMany(k => k.FileStreams).WithOne(k => k.Merchant).HasForeignKey(k => k.MerchantFid);
                entity.HasMany(k => k.Informations).WithOne(k => k.Merchant).HasForeignKey(k => k.MerchantFid);
            });

            modelBuilder.Entity<YachtMerchantProductSuppliers>(entity =>
            {
                entity.HasOne(k => k.Product).WithMany(k => k.ProductSuppliers).HasForeignKey(k => k.ProductFid);
                entity.HasOne(k => k.Vendor).WithMany(k => k.ProductSuppliers).HasForeignKey(k => k.VendorFid);
            });

            modelBuilder.Entity<YachtMerchantProductInventories>(entity =>
            {
                entity.HasMany(k => k.ProductPricings).WithOne(k => k.Product).HasForeignKey(k => k.ProductFid);
            });

            modelBuilder.Entity<YachtAdditionalServiceDetails>(entity =>
            {
                entity.HasOne(k => k.AdditionalService).WithMany(k => k.AdditionalServiceDetails).HasForeignKey(k => k.AdditionalServiceFid);
                entity.HasOne(k => k.Product).WithMany(k => k.AdditionalServiceDetails).HasForeignKey(k => k.ProductFid);
            });

            modelBuilder.Entity<YachtAdditionalServiceControls>(entity =>
            {
                entity.HasOne(k => k.Yacht).WithMany(k => k.AdditionalServiceControls).HasForeignKey(k => k.YachtFid);
                entity.HasOne(k => k.AdditionalService).WithMany(k => k.AdditionalServiceControls).HasForeignKey(k => k.AdditionalServiceFid);
            });

            modelBuilder.Entity<YachtPricingPlans>(entity =>
            {
                entity.HasMany(k => k.Details).WithOne(k => k.PricingPlan).HasForeignKey(k => k.PricingPlanFid);
                entity.HasMany(k => k.Informations).WithOne(k => k.PricingPlan).HasForeignKey(k => k.PricingPlanFid);
                entity.HasOne(k => k.Port).WithMany(k => k.PricingPlans).HasForeignKey(k => k.YachtPortFid);
                entity.HasOne(k => k.Yacht).WithMany(k => k.PricingPlans).HasForeignKey(k => k.YachtFid);
            });

            modelBuilder.Entity<YachtCharterings>(entity =>
            {
                entity.HasOne(k => k.Yacht)
                      .WithMany(k => k.Charterings)
                      .HasForeignKey(k => k.YachtFid);

                entity.HasMany(k => k.CharteringDetails)
                      .WithOne(k => k.Chartering)
                      .HasForeignKey(k => k.CharteringFid);
            });

            modelBuilder.Entity<YachtMerchantInformations>()
                .HasMany(k => k.InformationDetails)
                .WithOne(k => k.Information)
                .HasForeignKey(k => k.InformationFid);
            ...
            modelBuilder.Entity<YachtCounters>()
                .HasOne(k => k.Yacht)
                .WithOne(k => k.Counter)
                .HasForeignKey<YachtCounters>(k => k.YachtId);
            ...
            modelBuilder.Entity<YachtAttributeValues>()
                .HasOne(k => k.Attribute)
                .WithMany(k => k.AttributeValues)
                .HasForeignKey(k => k.AttributeFid);
            ...
            modelBuilder.Entity<YachtPorts>()
                .HasOne(k => k.PortLocation)
                .WithMany(k => k.YachtPorts)
                .HasForeignKey(k => k.PortFid);
            #endregion
